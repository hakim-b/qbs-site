...
export function Aurora({
  colorStops = ["#0171bb", "#78be20", "#0693e3"],
  amplitude = 1,
  blend = 0.5,
  speed = 1,
  time,
  ...
  useEffect(() => {
    ...
    const initialStops = initial.colorStops ?? [
      "#0171bb",
      "#78be20",
      "#0693e3",
    ];
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColorStops: { value: initialStops.map(hexToRgb) },
    ...
    const renderFrame = (timestamp: number) => {
      ...
      const nextTime = current.time ?? timestamp * 0.01;
      program.uniforms.uTime.value = nextTime * (current.speed ?? 1) * 0.1;
      ...
      program.uniforms.uColorStops.value = (
        current.colorStops ?? initialStops
      ).map(hexToRgb);
